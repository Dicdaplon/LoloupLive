...
// Chemins directs
import logoUrl from "../data/HDexportLogo.png";
  ...
  useEffect(() => {
    ...
    const sketch = (p: p5) => {
      ...
      class CircleButton {
        ...
        constructor(
          public label: string,
          public link: string, // route React
          public x: number,
          public y: number,
          public c: p5.Color
        ) {}
      ...
      // p5 v2: tout charger en setup (async)
      p.setup = async () => {
        ...
        const [/*logoImg*/, camImg] = await Promise.all([
          p.loadImage(logoUrl),
          p.loadImage(cameraUrl),
        ]);
        cameraIcon = camImg;

        // Routes React au lieu de .html
        buttons = [
      ...
      p.draw = () => {
        ...
        p.textSize(36);
        p.fill(0, 180);
        p.text("Loloup Jam LIVE", p.width / 2 + 2, 34);
        p.fill(255);
    ...
    return () => {
      if (p5Ref.current) {
        if ((p5Ref.current as any)._onRemove) (p5Ref.current as any)._onRemove();
