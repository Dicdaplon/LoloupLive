...
const AccueilSketch: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const sketch = (p: p5) => {
      // --- classes d'abord (évite tout souci de typing/référence) ---
      class CircleButton {
        baseSize = 100;
        ...
        display() {
          ...
          p.push(); p.noStroke();
          p.fill(255,0,0,100);   p.ellipse(this.x + 2, this.y - 1, size);
          p.fill(0,255,255,100); p.ellipse(this.x - 2, this.y + 1, size);
          p.pop();

          p.fill(this.c); p.ellipse(this.x, this.y, size);

          p.push();
          p.fill(0,180); p.text(this.label, this.x + 2, this.y + 2);
          p.fill(255);   p.text(this.label, this.x, this.y);
          p.pop();
        }
      ...
      class MinorButton {
        ...
        display(){
          ...
          const jx = p.random(-0.8,0.8), jy = p.random(-0.8,0.8);

          p.push(); p.noFill(); p.strokeWeight(1.5);
          for (let i=3;i>=1;i--){
          ...
          p.push(); p.stroke(this.c); p.strokeWeight(1.5); p.noFill();
          p.ellipse(this.x, this.y, size); p.pop();
      ...
      function repositionButtons(){
        ...
        if (!isPortrait){
          buttons[0].x = p.width*0.25; buttons[0].y = p.height/2;
          buttons[1].x = p.width*0.41; buttons[1].y = p.height/2;
          buttons[2].x = p.width*0.57; buttons[2].y = p.height/2;
          buttons[3].x = p.width*0.73; buttons[3].y = p.height/2;
          buttons[4].x = p.width*0.88; buttons[4].y = p.height/1.4;
        } else {
          buttons[0].x = p.width/2; buttons[0].y = p.height*0.35;
          buttons[1].x = p.width/2; buttons[1].y = p.height*0.51;
          buttons[2].x = p.width/2; buttons[2].y = p.height*0.67;
          buttons[3].x = p.width/2; buttons[3].y = p.height*0.83;
          buttons[4].x = p.width*0.83; buttons[4].y = p.height/1.69;
        }
      }
      ...
      p.setup = () => {
        ...
        (canvas.elt as HTMLCanvasElement).setAttribute("data-homebuttons", "1"); // pour cleanup hard
        ...
        // 🔁 charge l'icône caméra mais ne bloque PAS l'UI
        p.loadImage(
          cameraUrl,
          (img) => { cameraIcon = img; },
        ...
        // Boutons immédiatement (pas d'attente image)
        buttons = [
      ...
      p.draw = () => {
        p.clear();
        if (!ready) {
          p.fill(255); p.text("Chargement…", p.width/2, p.height/2);
          return;
        }
        ...
        for (const b of buttons){ b.update(); b.display(); }
      };
      ...
      (p as any)._onRemove = () => window.removeEventListener("orientationchange", onOrient);
    };
