...
function validateChordDoc(data: unknown): { ok: true; value: ChordDoc } | { ok: false; errors: string[] } {
  const errs: string[] = [];
  const d = data as Partial<ChordDoc>;
  if (!d || typeof d !== "object") return { ok: false, errors: ["Payload invalide (doit être un objet)."] };

  if (typeof d.titre !== "string" || !d.titre.trim()) errs.push("champ 'titre' requis (string non vide).");

  if (!Array.isArray(d.grille)) {
    errs.push("champ 'grille' requis (array).");
  } else {
    d.grille.forEach((blk, i) => {
      if (!blk || typeof blk !== "object") { errs.push(`grille[${i}] doit être un objet.`); return; }
      if (!isStringArray(blk.mesures)) errs.push(`grille[${i}].mesures doit être un tableau de strings.`);
      if (blk.repetitions != null && typeof blk.repetitions !== "number") errs.push(`grille[${i}].repetitions doit être un number (optionnel).`);
    });
  }
  ...
  (["style","tonalite","difficulte","signature"] as const).forEach((k) => {
    const v = d[k]; if (v != null && typeof v !== "string") errs.push(`${k} doit être un string.`);
  });
...
export default function UploadChordsRTDB() {
  ...
  const doValidate = useCallback(() => {
    ...
    try {
      ...
      if (!res.ok) { setErrors(res.errors); setParsed(null); return; }
      setErrors([]); setParsed(res.value);
  ...
  const doUpload = useCallback(async () => {
    ...
    if (!id) { setErrors(["L'ID du document est vide."]); return; }
    if (/[.#$\[\]]/.test(id)) { setErrors(["ID invalide : ne doit pas contenir . # $ [ ]"]); return; }
    ...
    try {
      ...
      if (!overwrite) {
        ...
        if (snap.exists()) { setErrors([`Le morceau '${id}' existe déjà.`]); setBusy(false); return; }
      }
    ...
    <main className="u-container">
      ...
      <section className="u-panel">
        ...
        <div className="u-row">
          ...
          <textarea
            value={text}
            onChange={(e) => { setText(e.target.value); setParsed(null); setErrors([]); setResultMsg(""); }}
            rows={14}
            spellCheck={false}
        ...
        <div className="u-row u-row-inline">
          <div className="u-field">
            ...
            <input value={docId} onChange={(e) => setDocId(e.target.value)} placeholder="(auto: slug du titre)" />
          </div>
        ...
        <div className="u-row u-row-inline">
          <button onClick={doValidate} className="u-btn">Valider</button>
          <button
            onClick={doUpload}
                ...
                <div key={row.i} className="u-line">
                  ...
                  <div className="u-measures">
                    {row.measures.flatMap((m, i) =>
                      m.split("\\").map((h, j) => (
                        <span key={`${i}-${j}`} className="u-chip">{h.trim()}</span>
                      ))
                    )}
                  </div>
                </div>
      ...
      <section className="u-panel">
        ...
        <p>
          Colle un texte au <b>format JSON</b> qui décrit ta grille. Utilise “Valider” pour
          vérifier la syntaxe et la forme. Règles simples :
        </p>
        <ul>
          <li><b>titre</b> (obligatoire) : nom du morceau.</li>
          <li><b>grille</b> (obligatoire) : liste de blocs, chaque bloc a <code>mesures</code> (ex: <code>["Am","Dm","E7","Am"]</code>)
              et optionnellement <code>repetitions</code> (combien de fois répéter le bloc).</li>
          <li>Pour une <b>demi-mesure</b>, sépare avec un antislash <code>"C\\G"</code> → affichera 2 demi-mesures.</li>
          <li>Champs optionnels : <code>style</code>, <code>bpm</code>, <code>tonalite</code>, <code>difficulte</code>, <code>signature</code>.</li>
        </ul>

        <p>Outils utiles (ouvrent dans un nouvel onglet) :</p>
        <ul>
          <li><a href="https://jsonlint.com" target="_blank" rel="noreferrer">JSONLint</a> — vérifie la syntaxe.</li>
          <li><a href="https://jsonformatter.org" target="_blank" rel="noreferrer">JSON Formatter</a> — met en forme/valide.</li>
        </ul>
