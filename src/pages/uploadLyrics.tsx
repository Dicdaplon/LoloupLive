...
export default function UploadLyricsRTDB() {
  ...
  const doSend = useCallback(async () => {
    ...
    if (!finalId) {
      setErr('ID vide (remplis Titre ou ID).');
      return;
    }
    if (/[.#$\[\]]/.test(finalId)) {
      setErr('ID invalide : interdit . # $ [ ]');
      return;
    }
    if (!lyrics.trim()) {
      setErr('Paroles vides.');
      return;
    }
        ...
        <div className="u-row u-row-inline">
          ...
          <div className="u-field">
            ...
            <div className="u-hint">
              ID utilisé: <code>{suggestedId || '(vide)'}</code>
            </div>
          </div>
        </div>

        <div className="u-row u-row-inline">
          <button className="u-btn" onClick={onPickFile}>
            Choisir un .txt
          </button>
